...
#------submit_FSL.py---------------------------
#
# This script is used for BET job submission with python
#
...
template_f = file("AHABII_BET_TEMPLATE.sh")  #job template location (on head node)
...
subnums = ["30634_scandata","31684_scandata","31754_scandata","32366_scandata","32950_scandata","33047_scandata","36746_scandata","36900_scandata","36998_scandata","37795_scandata","38694_scandata","38888_scandata","39223_scandata","40897_scandata","40948_scandata","41762_scandata","42069_scandata","42470_scandata","43437_scandata","45540_scandata","46531_scandata","47019_scandata","48508_scandata","52145_scandata","52614_scandata","52836_scandata","53006_scandata","54642_scandata","54742_scandata","55435_scandata","56153_scandata","56160_scandata","57677_scandata","58112_scandata","59751_scandata","59946_scandata"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1] #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
anat = "inplanes_t2_256x256.6"  #this is the anatomical folder
output = "anat_brain_noeyes" #the name you want for the output file in the BET folder
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ANATFOLD_SUB", str(anat) )
		...
		while n_jobs >= maintain_n_jobs: 
			...
			if n_jobs >= maintain_n_jobs: 
				...
				cmd = "qsub -v EXPERIMENT=%s %s inplanes_t2*"  % ( experiment, tmp_job_fname )
