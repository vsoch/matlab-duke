...
ORIGIN=SUB_ORIGINSET_SUB        # yes means that we are manually setting the origin
XCOORD=SUB_XCOORD_SUB             # the x coordinate to set the origin to
YCOORD=SUB_YCOORD_SUB             # the x coordinate to set the origin to
ZCOORD=SUB_ZCOORD_SUB             # the x coordinate to set the origin to
...
if [ $ANATFOLDER != 'none' ]; then

[ -e "$ANATDIRECTORY" ]; then
...
for i in 'spm_batch1origin.m'; do
sed -e 's@SUB_BIACROOT_SUB@'$BIACROOT'@g' \
 ...
 -e 's@SUB_SETORIGIN_SUB@'$ORIGIN'@g' \
 -e 's@SUB_X_SUB@'$XCOORD'@g' \
 -e 's@SUB_Y_SUB@'$YCOORD'@g' \
 -e 's@SUB_Z_SUB@'$ZCOORD'@g' \
 -e 's@SUB_TFOLDER_SUB@'$TFOLD'@g' <$i> $OUTDIR/spm_batch1origin_${RUN}.m
done
 
# Change to output directory and run matlab on input script
cd $OUTDIR

/usr/local/matlab2009b/bin/matlab -nodisplay < spm_batch1origin_${RUN}.m
...
# Loop through template script replacing keywords
for i in 'spm_batch2origin.m'; do
sed -e 's@SUB_MOUNT_SUB@'$EXPERIMENT'@g' \
 ...
 -e 's@SUB_SUBJECT_SUB@'$SUBJ'@g' <$i> $OUTDIR/spm_batch2origin_${RUN}.m
done
...
/usr/local/matlab2009b/bin/matlab -display :$RANDINT < spm_batch2origin_${RUN}.m
