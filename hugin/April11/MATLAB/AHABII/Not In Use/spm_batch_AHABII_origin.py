...
# should be both set to "no." If you want to set the origin for all images (due to poor
# consistency of origins in AHAB data) then set "setorigin" to "yes."
# If you do it manually, before realigning AC-PC delete subject folders under Processed
...
template_f = file("spm_batch_AHABII_origin_template.sh")      #job template location (on head node)
...
subnums = ["30337_scandata"] #should be entered in quotes, separated by commas, to be used as strings
...
oldfacesrun = 'yes'          #"yes" runs preprocessing and single subject analysis for old faces, "no" skips
...
setorigin="yes"              #If you want to set the origin, indicate "yes"
xcoord="126"    
ycoord="165"
zcoord="12"    
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ORIGINSET_SUB", str(setorigin) )
		tmp_job_file = tmp_job_file.replace( "SUB_XCOORD_SUB", str(xcoord) )
		tmp_job_file = tmp_job_file.replace( "SUB_YCOORD_SUB", str(ycoord) )
		tmp_job_file = tmp_job_file.replace( "SUB_ZCOORD_SUB", str(zcoord) )
