...
subnums = ["20110314_12429"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1]                   #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
anatfolder = "series400"     #folder under Data/Anat that includes the anatomical dicom images
tone = "series300"           #t1 folder (series005) if it doesn't exist, leave blank
facesfolder = "run006_02"    #folder with the faces functional data (in V00*.img/.hdr format)
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_fname = "_".join( ["ADOLREG_SPM", subnum, runstr ] )
