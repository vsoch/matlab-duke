...
template_f = file("spm_preprocess_TEMPLATE.sh") #job template location (on head node)
...
anatfolder = "series002"     #this is the folder under Data/Anat that includes the anatomical dicom images
funcfolder = "run004_03"     #this is the name of the folder with the functinoal data to convert
task = "faces"               #This is the name of the functional task ("cards" or "faces")

# processing choices
# TO INSERT IN SCRIPTS - FUNCTIONALITY TO CHOOSE ANALYSIS WE WANT RUN
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ANATFOL_SUB", str(anatfolder) )
 		tmp_job_file = tmp_job_file.replace( "SUB_FUNCFOL_SUB", str(funcfolder) )
		...
		tmp_job_fname = "_".join( ["SPM_PREPROCESS", subnum, runstr ] )
