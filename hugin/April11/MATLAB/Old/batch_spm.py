...
#------SPM_BATCH.py---------------------------
#
# This script is used for converting dicom to nifti (for anatomical data) 
# and 3D --> 4D images (for functional data) and all steps of preprocessing
# in SPM.  When complete, output should be visually checked and ART run
#
...
template_f = file("batch_spm_TEMPLATE.sh")      #job template location (on head node)
ppathdefs = "/home/vvs4/matlab"                 #location of personal path defs on qinteract
...
subnums = ["TEST91"]         #should be entered in quotes, separated by commas, to be used as strings
...
t1 = "series005"             #t1 folder (series005) if it doesn't exist, leave blank
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_T1FOLD_SUB", str(t1) )
		tmp_job_file = tmp_job_file.replace( "SUB_PPDEFS_SUB", str(ppathdefs) )
