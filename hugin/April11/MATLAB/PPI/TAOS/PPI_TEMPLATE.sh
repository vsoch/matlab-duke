...
# TAOS_PPI (in Scripts/MATLAB/PPI folder on Munin) to run PPI analysis.  It
...
sed -e 's@SUB_MOUNT_SUB@'$EXPERIMENT'@g' \
 ...
 -e 's@SUB_SUBJECT_SUB@'$SUBJ'@g' <$i> $OUTDIR/TAOS_PPI_${RUN}.m
 done
...
/usr/local/bin/matlab -display :$RANDINT < TAOS_PPI_${RUN}.m;

echo "Done running TAOS_PPI.m in matlab"
