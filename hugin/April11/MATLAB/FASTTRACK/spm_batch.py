...
experiment = "FastTrack.01"                     #experiment name for qsub
...
subnums = ["20110326_12530"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1]                   #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
anatfolder = "series200"     #folder under Data/Anat that includes the anatomical dicom images
...
orderno = "3"                #This is the group of subjects order number (must submit 1 python / order number)
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_fname = "_".join( ["FASTTRACK", subnum, runstr ] )
