...
template_f = file("SPM_REG_Check_TEMPLATE.sh")      #job template location (on head node)
...
jobname = ["Faces"] # Name for the job
subjectlist = "20100125_10274 20100125_10276 20100128_10293 20100129_10298 20100129_10299" # list of all subjects, separated by one space, no commas or anything else
runs = [1]               #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)

# processing choices
facescheck = "yes"    #folder with the faces functional data (in V00*.img/.hdr format)
cardscheck = "no"    #folder with the cards functional data (in V00*.img/.hdr format)
restcheck = "no"     #folder with the rest functional data (in V00*.img/.hdr format)
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!###############################################
...
for subnum in jobname:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_SUBNUM_SUB", str(subjectlist) )
		tmp_job_file = tmp_job_file.replace( "SUB_FACESCHECK_SUB", str(facescheck) )
		tmp_job_file = tmp_job_file.replace( "SUB_CARDSCHECK_SUB", str(cardscheck) )
		tmp_job_file = tmp_job_file.replace( "SUB_RESTCHECK_SUB", str(restcheck) )
 		 
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["SPM_REG", jobname, runstr ] )
