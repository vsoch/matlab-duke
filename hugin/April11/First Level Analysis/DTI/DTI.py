...
template_f = file("DTI_TEMPLATE.sh")  #job template location (on head node)
...
subnums = ["47341_scandata"] #should be entered in quotes, separated by commas, to be used as strings
runs = [1] #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
dtifolder = "dti_896x896.23" #the folder containing the dti nifti, bvec, and bvals files
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_fname = "_".join( ["DTI", subnum, runstr ] )
