...
template_f = file("first_level_FEAT_FIGS_T1_TEMPLATE.sh")  #job template location (on head node)
...
subnums = ["040719095232"] #should be entered in quotes, separated by commas, to be used as strings
...
anatdir = "4" #the folder name that contains the anatomical data
funcdir = "5" #the folder name that contains the functional data
designvar = "SplitBlockThree" #the name of the experiment design (for FIGS, 4Block, 4BlockMixed, Individual, etc)
designtype = "SplitBlockThreeT1IntermedReg" #the name of the design type under FACES
tone = "8" # the folder that contains the T1 to use
####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###############################################
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_DESIGNTYPE_SUB", str(designtype) )
                tmp_job_file = tmp_job_file.replace( "SUB_T_ONE_NAME_SUB", str(tone) )
