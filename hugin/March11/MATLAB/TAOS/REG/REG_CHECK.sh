...
# after an SPM analysis and takes in a mask that the user intendes to use
# for analysis.  It also takes in a percent coverage specicification, and
# creates a list of subjects that have that level of coverage for the mask,
# and another list of subjects that do not.  It then prepares a uniquemask.nii 
# image that includes the subjects with passing coverage that can be used 
...
ROI=$EXPERIMENT/Analysis/SPM/ROI/VS/vs.img   # This is the file path to the
mask image ROI
