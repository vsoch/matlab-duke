function spm_change_paths(oldpath,newpath,slash)
%
...
function change_swd(oldpath,newpath,slash,old_slash)
 load SPM.mat SPM;
 size_swd = size(SPM.swd);
 size_oldpath = length(oldpath);
 size_newpath = length(newpath);
 new_size_swd = size_swd(1,2) - size_oldpath + size_newpath;
   for i = 1:new_size_swd
      if i <= size_newpath
          ...
          temp(i) = SPM.swd(size_oldpath-size_newpath+i);
          if temp(i) == old_slash
              temp(i) = slash;
          end;
      end
