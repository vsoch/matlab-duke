...
FACESORDER=SUB_ORDERNO_SUB      # this is the order number for the group of subjects
...
cd $EXPERIMENT/Data/Func/$SUBJ/run004_03/NiftiHeaders
mkdir -p images
...
NEWIMGPATH="V"$IMGPRE
NEWHDRPATH="V"$HDRPRE
cp *0$c.img images/$NEWIMGPATH
cp *0$c.hdr images/$NEWHDRPATH
done
...
NEWIMGPATH="V"$IMGPRE
NEWHDRPATH="V"$HDRPRE
cp *000$c.img images/$NEWIMGPATH
cp *000$c.hdr images/$NEWHDRPATH
done
...
cd $EXPERIMENT/Data/Func/$SUBJ/run004_02/NiftiHeaders
mkdir -p images
...
IMGFILE=*$c.img
HDRFILE=*$c.hdr
IMGPRE=$(echo $IMGFILE | cut -c13-20)
HDRPRE=$(echo $HDRFILE | cut -c13-20)
NEWIMGPATH="V"$IMGPRE
NEWHDRPATH="V"$HDRPRE
cp *$c.img images/$NEWIMGPATH
cp *$c.hdr images/$NEWHDRPATH
