...
cd $EXPERIMENT/Analysis/SPM/Processed
mkdir -p $SUBJ 
...
sed -e 's@SUB_PATHDEFS_SUB@'$PATHDEFS'@g' \
 ...
 -e 's@SUB_T1FOLDER_SUB@'$T1FOLD'@g' <$i> $OUTDIR/spm_batch1_${RUN}.m
done
 
# Change to output directory and run matlab on input script
cd $OUTDIR

/usr/local/matlab2009a/bin/matlab -nodisplay < spm_batch1_${RUN}.m
...
/usr/local/matlab2009a/bin/matlab -nodisplay < spm_batch2_${RUN}.m
