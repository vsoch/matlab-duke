...
% SUB_MATRIX_SUB feeds in the order number from the higher level scripts
% Th bash script checks that the order number is 1,2,3, or 4.
% The format of these matrices is [condition#,session#,weight]
...
% To process faces for multiple design types, we must prepare PPI for
% each of 1) Shapes, 2) All Faces, 3) Angry, 4) Surprise, 5) Fearful, 6)
% Neutral.  ALL of these will get fed into the single subject analysis as
% regressors, however the PPI.Y only needs to go in once.
if strcmp('SUB_TASK_SUB','faces')
