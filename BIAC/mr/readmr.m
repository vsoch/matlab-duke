...
%   "dimsizes" is a numeric array specifying the sizes of the dimensions in
%   the data, in order from fastest-moving to slowest-moving.
%   "pixelType" must be a supported MATLAB data type (see "help datatypes").
%   "byteOrder" must be 'l' (little-endian) or 'b' (big-endian).
%   "opt_dimorder" is an optional cell array of strings specifying the labels
%   of the dimensions in the data, e.g. { 'x', 'y', 'z', 't' } (this is the
%   default).
%   "opt_recSz" indicates how many bytes to read ([] or Inf means to read the
%   rest of the file, which is the default behavior).
...
% CVSId = '$Id: readmr.m,v 1.85 2009/04/23 12:48:49 gadde Exp $';
% CVSRevision = '$Revision: 1.85 $';
% CVSDate = '$Date: 2009/04/23 12:48:49 $';
...
try
  ...
  if isempty(info)
    ...
    switch mrparams.format
     ...
     otherwise
      ...
      % first download .img for Analyze/NIFTI if needed
      if strcmp(mrparams.format, 'Analyze7.5_SPM') | ...
            strcmp(mrparams.format, 'NIFTI-1')
        if ~isempty(downloaded{1})
          extstart = [];
          if length(inputlocs{1}) >= length('.nii')
            extstart = findstr(inputlocs{1}, '.nii');
          end
          if isempty(extstart)
            extstart = [];
      ...
      if isfield(newparams, 'direction')
        mrparams.direction = newparams.direction;
      end
      ...
      if ((strcmp(mrparams.format,'Analyze7.5_SPM') | ...
        ...
        if length(newparams.matSz) ~= 3
          emsg = sprintf('Can''t read multiple 4-D Analyze/NIFTI-1 files!'); error(emsg);
        end
        mrparams.matSz(4) = length(inputlocs);
  ...
  if isempty(info)
    ...
    if length(typesize) == 0
      typelist = strcat({', '}, {typesizes.type});
      typelist = strcat(typelist{:});
      typelist(1) = ' ';
      emsg = sprintf(['Unrecognized pixel type "' mrparams.pixelType '".\nMust be one of:%s'], typelist);
    end
    elemsize = typesize.size;
    if strcmp(mrparams.byteOrder, 'l') == 0 & ...
          strcmp(mrparams.byteOrder, 'b') == 0
      error(['Unrecognized byte order ''' mrparams.byteOrder '''.  Must be ''l'' or ''b''']);
    end
    ...
    if ~isfield(mrparams, 'dimorder') | isempty(mrparams.dimorder)
      mrparams.dimorder = { 'x', 'y', 'z', 't', 'u', 'v', 'w' };
    ...
    % do frags
    if strcmp(mrparams.format, 'Analyze7.5_SPM') | strcmp(mrparams.format, 'NIFTI-1')
      ...
      for fragnum=startfragnum:endfragnum
        ...
        if ~isempty(readfrag)
          ...
          if lastfid ~= -1 & strcmp(lastfilename,readfilename)
            ...
            msg = '';
            msg2 = '';
            rflen = length(readfilename);
            if rflen > 3 & readfilename(rflen-1:end) == 'gz'
              [fid,msg2] = gzopenlocation(readfilename, {'r', info.byteorder}, info.basepath);
              if fid == -1
              ...
              [fid,msg] = fopenlocation(readfilename, {'r', info.byteorder}, info.basepath);
              if fid == -1
                [fid,msg2] = gzopenlocation([readfilename '.gz'], {'r', info.byteorder}, info.basepath);
              end
            end
            if ~isnumeric(fid)
              isjavafid = 1;
          ...
          if isempty(postselect)
            if isjavafid
              % to reduce memory copies/usage, read compressed data in smaller
              % chunks
              readsize = 1024*1024;
              tmppos = srspos;
              while tmppos < srspos + elemsinfrag
                if tmppos + readsize > srspos + elemsinfrag
                  readsize = srspos + elemsinfrag - tmppos;
                end
                srs(tmppos:tmppos+readsize-1) = ...
                    fread(fid, readsize, [info.rawelemtype '=>' info.elemtype]);
                tmppos = tmppos + readsize;
              end
            else
            ...
            if isjavafid
              ...
              readsize = 1024*1024;
              tmppos = pagepos;
              while tmppos < pagepos + elemsinfrag
                if tmppos + readsize > pagepos + elemsinfrag
                  readsize = pagepos + elemsinfrag - tmppos;
                end
                srs(tmppos:tmppos+readsize-1) = ...
                    fread(fid, readsize, [info.rawelemtype '=>' info.elemtype]);
                tmppos = tmppos + readsize;
...
% Fix gzipped lengths if needed
for i=1:length(files)
  [pathstr,name,ext,vers] = fileparts(files{i});
  if strcmp(ext, '.gz')
    [fid,emsg]=fopenlocation(files{i}, {'r','l'});
    if fid==-1, error(emsg); end
    % seek to 4 bytes before the end of file
    fseek(fid, -4, 'eof');
    fsizes(i) = fread(fid, 1, 'uint32=>double');
    fclose(fid);
  end
end
return
...
if finfo.ismosaic
  info.dimensions = struct( ...
      ...
      'units', NaN, ...
      'size', {double(acqcols), double(dinfo.Columns/acqcols), double(acqrows), double(length(inputfiles) * (dinfo.Rows/acqrows))}, ...
  ...
  info.dimensions = struct( ...
      'type', {'x','y','z'}, ...
      'units', NaN, ...
      'size', {double(dinfo.Columns), double(dinfo.Rows), length(inputfiles)}, ...
      'outputselect', NaN, ...
      'origin', NaN, ... % rest will be filled in later
      'spacing', NaN, ...
      'gap', NaN, ...
...
%
% $Log: readmr.m,v $
% Revision 1.85  2009/04/23 12:48:49  gadde
% Add potential dimensions beyond 4
%
% Revision 1.84  2007/04/04 18:33:50  gadde
% readmrhdr returns origin as a cell array, so use {} to index it.
%
% Revision 1.83  2006/10/13 22:16:16  gadde
% If frag file name ends in "gz" (not just ".gz"), try gzopen first,
% then regular fopen.
%
% Revision 1.82  2006/09/18 21:09:04  gadde
% NIfTI-1 and compressed NIfTI-1 fixes for readmr and readmrgui
%
% Revision 1.81  2006/07/24 21:29:26  gadde
% Fix for NIfTI-1 reading.
%
% Revision 1.80  2006/07/19 20:23:42  gadde
% Clean up logic for deciding whether or not frag reads are using java
% interface or not.
%
% Revision 1.79  2006/07/19 17:17:20  gadde
% Make sure sizes returned by DICOM files are represented as doubles
%
% Revision 1.78  2006/07/18 21:46:11  gadde
% More updates for reading compressed data.
%
% Revision 1.77  2006/07/14 21:20:20  gadde
% Updates for gzip reading
%
% Revision 1.76  2006/06/27 20:41:52  gadde
% Updates to support reading compressed frags.
%
% Revision 1.75  2006/06/02 21:43:42  gadde
% NIFTI-1 updates.
%
% Revision 1.74  2006/01/18 15:44:28  gadde
% Updates to help and error messages.
%
