...
% CVSId = '$Id: writemr.m,v 1.34 2007/02/23 19:50:56 gadde Exp $';
CVSRevision = '$Revision: 1.34 $';
% CVSDate = '$Date: 2007/02/23 19:50:56 $';
...
info = mrstruct.info;
args(1) = [];
frags = struct('filenames', {{}}, 'fileoffsets', {[]}, 'fragsizes', {[]});
bxhtype = 'image';
bxhsubtype = '';
...
if strcmp('Raw', outputformat)
  ...
  frags.filenames{1} = fullfilename;
  frags.fileoffsets(1) = NaN;
  frags.fragsizes(1) = NaN;
...
elseif strcmp('Float', outputformat) | ...
  ...
  for vnum = 1:sizes(4)
    ...
    % set up frags for BXH output if needed
    fragnum = length(frags.filenames) + 1;
    frags.filenames{fragnum} = volfilename;
    frags.fileoffsets(fragnum) = NaN;
    frags.fragsizes(fragnum) = NaN;
  end
...
elseif strcmp('BXH', outputformat)
  ...
  frags.filenames{1} = fulldatafilename;
  frags.fileoffsets(1) = NaN;
  frags.fragsizes(1) = NaN;
else
...
if ~nobxh | strcmp('BXH', outputformat)
  ...
  for i = 1:length(frags.filenames)
    if isrelpath(frags.filenames{i})
      frags.filenames{i} = [pwd filesep frags.filenames{i}];
    end
    [fragpathstr, fragname, fragext, fragver] = fileparts(frags.filenames{i});
    if strcmp(fragpathstr, bxhpathstr)
      frags.filenames{i} = [fragname fragext fragver];
    end
  end
  ...
  for i = 1:length(frags.filenames)
    frags.fileoffsets(i) = 0;
    frags.fragsizes(i) = fragsize;
  end
...
% $Log: writemr.m,v $
% Revision 1.34  2007/02/23 19:50:56  gadde
% Make sure to write all filenames to .bxh file for Volume case.
%
% Revision 1.33  2006/06/29 20:17:20  gadde
