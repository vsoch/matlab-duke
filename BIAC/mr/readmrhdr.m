...
% CVSId = '$Id: readmrhdr.m,v 1.27 2006/09/25 15:03:55 gadde Exp $';
% CVSRevision = '$Revision: 1.27 $';
% CVSDate = '$Date: 2006/09/25 15:03:55 $';
...
try
  ...
  case {'Analyze7.5_SPM', 'NIFTI-1'}
    % Read a Analyze 7.5 file with SPM modifications, or a NIFTI-1 file.
    %
    % Assumptions:
    ...
    %  NIfTI-1 format as specified in nifti1.h, available at:
    ...
    if strcmp(ext, '.gz')
      [fid,msg] = gzopenlocation(hdrFilename, {'r', 'l'});
    else
      [fid,msg] = fopenlocation(hdrFilename, {'r', 'l'});
    end
    ...
    elseif hdrsizeBigEndian == 348
      ...
      fclose(fid);
      if strcmp(ext, '.gz')
        [fid,emsg] = gzopenlocation(hdrFilename, {'r', 'b'});
      else
      ...
      fclose(fid);
      emsg = sprintf('File %s is not an Analyze7.5, SPM, or NIfTI-1 file',hdrFilename); error(emsg);
    end
...
%    if (all(char(magic(1:3))' == 'ni1') | all(char(magic(1:3))' == 'n+1')) & magic(4) == 0
%      isnifti = 1;
%    end
    % Don't check NIFTI magic number, just believe the format parameter.
    % Otherwise, we get into trouble with backwards seeks if you try
    % to verify NIFTI-ness of a compressed NIFTI file.
    isnifti = 0;
    if strcmp(format, 'NIFTI-1')
      isnifti = 1;
    end
    
    % make sure to read values out of header in order so as not to
    % seek backwards.
    ...
    if isnifti
      % grab nifti-specific info
      fseek(fid, 76, 'bof'); % pixdim[0]
      qfac = fread(fid, 1, 'float32');
    end
    ...
    if isnifti
      fseek(fid, 108, 'bof');
      params.hdrSz = fread(fid, 1, 'float32');
    end
    ...
    if isnifti
      fseek(fid, 252, 'bof');
      qformcode = fread(fid, 1, 'int16');
      sformcode = fread(fid, 1, 'int16');
      qb = fread(fid, 1, 'float32');
      qc = fread(fid, 1, 'float32');
      qd = fread(fid, 1, 'float32');
      qx = fread(fid, 1, 'float32');
      qy = fread(fid, 1, 'float32');
      qz = fread(fid, 1, 'float32');
      srows = fread(fid, 12, 'float32');
    end
    ...
    if isnifti
      params.direction = { srows([1,5,9]), srows([2,6,10]), srows([3,7,11]) };
      params.origin = { srows(4), srows(8), srows(12) };
      if (sformcode == 0 & qformcode > 0)
        dx = 1; dy = 1; dz = 1;
        % from nifti1_io.c
        b = qb; c = qc; d = qd;
        a = 1.0 - (b*b + c*c + d*d);
        ...
        if (dx > 0.0), xd = dx; else, xd = 1.0; end
        if (dy > 0.0), yd = dx; else, yd = 1.0; end
        if (dz > 0.0), zd = dx; else, zd = 1.0; end
        if (qfac < 0.0), zd = -zd; end
        params.direction = { ...
            [       (a*a+b*b-c*c-d*d) * xd , ...
              2.0 * (b*c-a*d        ) * yd , ...
              2.0 * (b*d+a*c        ) * zd ], ...
            ...
            [ 2.0 * (b*d-a*c        ) * xd , ...
              2.0 * (c*d+a*b        ) * yd , ...
                    (a*a+d*d-c*c-b*b) * zd ] };
        params.origin = { qx, qy, qz };
      end
      params.direction{1} = params.direction{1} ./ sqrt(sum(params.direction{1} .^ 2));
      params.direction{2} = params.direction{2} ./ sqrt(sum(params.direction{2} .^ 2));
      params.direction{3} = params.direction{3} ./ sqrt(sum(params.direction{3} .^ 2));
      dirmat = [ params.direction{:} ];
      dirmat = reshape(dirmat, [3 3]);
      [maxvals,ii]=max(abs(dirmat));
      params.origin = params.origin(ii);
      [params.origin(end+1:numdims)] = deal({NaN});
      [params.direction(end+1:numdims)] = deal({[]});
    end
...
%
% $Log: readmrhdr.m,v $
% Revision 1.27  2006/09/25 15:03:55  gadde
% Declare isnifti in the correct scope.
%
% Revision 1.26  2006/09/18 21:09:05  gadde
% NIfTI-1 and compressed NIfTI-1 fixes for readmr and readmrgui
%
% Revision 1.25  2006/07/25 16:43:25  gadde
% In NIFTI-1, only use qform if sform doesn't exist.
%
% Revision 1.24  2006/07/24 21:30:01  gadde
% Fix for NIfTI-1 reading of qform matrix.
%
% Revision 1.23  2006/06/02 21:43:56  gadde
% NIFTI-1 updates.
%
