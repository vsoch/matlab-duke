...
% CVSId = '$Id: autodetectmr.m,v 1.30 2006/09/18 21:09:04 gadde Exp $';
% CVSRevision = '$Revision: 1.30 $';
% CVSDate = '$Date: 2006/09/18 21:09:04 $';
...
try
  ...
  if (strcmp(ext, '.gz'))
    [path,name,ext,vers] = fileparts(fullfile(path,[name vers]));
    ext = [ext '.gz'];
  end
  if strcmp(ext, '.nii') | ...
        strcmp(ext, '.nii.gz') | ...
    ...
    fid2 = fid;
    if ~strcmp(ext, '.hdr') & ~strcmp(ext, '.nii')
      if strcmp(ext, '.nii.gz')
        fid2 = gzopenlocation(filenames{1}, {'r', 'l'});
      else
    ...
    if chars(4) == 0 & ...
          ((strcmp(ext, '.nii.gz') & all(char(chars(1:3))' == 'n+1')) | ...
           (strcmp(ext, '.nii') & all(char(chars(1:3))' == 'n+1')) | ...
           (~strcmp(ext, '.nii') & all(char(chars(1:3))' == 'ni1')))
      format = 'NIFTI-1';
      fclose(fid);
      return
    end
  ...
  if magic(1) ~= -1 & ~isempty(findstr(magic, sprintf('\n'))) & ~isempty(findstr(magic, '<?xml'))
    while 1
      ...
      if findstr(magic, 'http://nbirn.net/Resources/Users/Applications/xcede')
        format = 'BXH';
        fclose(fid);
        return
      end
...
% Revision 1.30  2006/09/18 21:09:04  gadde
% NIfTI-1 and compressed NIfTI-1 fixes for readmr and readmrgui
%
% Revision 1.29  2006/06/02 21:43:49  gadde
% NIFTI-1 updates.
%
% Revision 1.28  2005/12/07 18:54:40  gadde
% Add XCEDE as "BXH".
%
