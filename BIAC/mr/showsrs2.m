...
%     overlayCfg.clusterSize is a scalar that describes number of acjacent
%       voxels used to define a 2D cluster
%     overlayCfg.maskString is a string that describes which other overlays
%       to mask current overlay with
%
...
% CVSId = '$Id: showsrs2.m,v 1.111 2009/04/23 18:58:02 bizzell Exp $';
% CVSRevision = '$Revision: 1.111 $';
% CVSDate = '$Date: 2009/04/23 18:58:02 $';
...
try
  ...
  elseif ~ischar(varargin{1})  % LAUNCH GUI
    ...
    % If delayed interpolation enabled, show in config menu
    if otherInfo.delayInterp, set(handles.UseDelayInterpMenu,'Checked','On'); end
    ...
    %                rasflag is a six-letter code indicating the orientation
    %                  of the data, or empty string if unknown
    %                otherImgAx holds handles to the 2 other image axes
    %                  created for 3-plane view (empty if single-plane)
    %                tsAx holds ahandle to the timeseries plot axes
    %                  created for 3-plane view (empty if single-plane)
    %
    ...
    figUD.linkedFigIDs = [];
    figUD.delayInterp = otherInfo.delayInterp;
    figUD.xMarkers = struct('value',[],'handle',[]);
    figUD.rasflag = otherInfo.rasflag;
    figUD.otherImgAx = []; % holds handles to other image axes for 3-plane view
    figUD.tsAx = []; % holds timeseries plot axis for 3-plane view
    ...
    %                raslabels is a vector of handles to the RAS labels (empty if orientation is
    %                     not specified)
    %
  ...
  elseif ischar(varargin{1}) % INVOKE NAMED SUBFUNCTION OR CALLBACK
    ...
    if any(strcmp(varargin{1},externalTasks))
      ...
      if any(strcmp(varargin{1},{'ChangeTimePoint_Callback','Play_Callback',...
          'ShowMarkersMenu_Callback','ShowTimePlot_Callback','AddXMarker_Callback'}))
        corrNumArgs = 3;
...
if ~isempty(figUD.rasflag)
  for dimind=[dim dim+3]
    raschar = figUD.rasflag(dimind);
    if raschar == 'R', raschar = 'L';
    elseif raschar == 'L', raschar = 'R';
    elseif raschar == 'A', raschar = 'P';
    elseif raschar == 'P', raschar = 'A';
    elseif raschar == 'S', raschar = 'I';
    elseif raschar == 'I', raschar = 'S';
    end
    figUD.rasflag(dimind) = raschar;
  end
  clear dimind;
  set(handles.fig, 'UserData',figUD);
end
...
% fix existing RAS labels
raslabels=sort(findobj(handles.fig,'Type','Text','Tag','RASLabel'));
for labelind=1:length(raslabels)
  label_h=raslabels(labelind);
  labelstr=get(label_h,'String');
  switch get(cbo,'Tag')
    case 'FlipMenuRL'
      if labelstr=='R', labelstr='L'; elseif labelstr=='L', labelstr='R'; end
    case 'FlipMenuAP'
      if labelstr=='A', labelstr='P'; elseif labelstr=='P', labelstr='A'; end
    case 'FlipMenuSI'
      if labelstr=='S', labelstr='I'; elseif labelstr=='I', labelstr='S'; end
  end
  set(label_h,'String',labelstr);
end
...
allAxes_h=[handles.imgAx, figUD.otherImgAx, figUD.tsAx];
...
for col=1:resizeInfo.axesMatrix(1)
  for row=1:resizeInfo.axesMatrix(2)
    ax_ud=get(allAxes_h(col,row),'UserData');
    if isfield(ax_ud,'raslabels') & ~isempty(ax_ud.raslabels)
      local_positionRASLabels(ax_ud.raslabels);
    end
    clear ax_ud;
  end
end
...
% --------------------------------------------------------------------
function AddXMarker_Callback(handles,showsrs2_h,cbo)
% Show X-marker on time plot
%
% AddXMarker_Callback(handles,showsrs2_h,cbo)
%      handles - structure of handles in showsrs2 window
%   showsrs2_h - handle of showsrs2 figure.  Included so that callback
%                switchyard works when cbo is not in showsrs2_h
%          cbo - handle of object which initiated the callback
...
% Bring up question dialog box
result=questdlg('Add or Delete X-Marker', ...
  'Add/Delete X-Marker','Add','Delete','Cancel','Cancel');
...
if ~isempty(result)
  ...
  switch lower(result)
    case 'add'
      % Open input dialog for placement of X-marker
      result2=inputdlg('Add X-Value:','Add X-Marker',1,{'0'});
      if ~isempty(result2)
        result2 = str2num(result2{1});
        [testval{1:length(fig_ud.xMarkers)}] = deal(fig_ud.xMarkers.value);
        % Find valid limits of X-marker
        if ~isempty(result2) & (result2 > 0) & (result2 <= max(fig_ud.timePts))
          if (length(fig_ud.xMarkers) == 1) & isempty(fig_ud.xMarkers(1).value) & ...
              isempty(fig_ud.xMarkers(1).handle)
            % No values set, set first value;
            fig_ud.xMarkers(1).value = result2;
            fig_ud.xMarkers(1).handle = [];
          elseif ~isempty(intersect(cell2mat(testval),result2));
            % Value exists already, don't add
          else
            % Values are set, save to next field
            idx = length(fig_ud.xMarkers) + 1;
            fig_ud.xMarkers(idx).value = result2;
            fig_ud.xMarkers(idx).handle = [];
          end
          set(handles.fig,'UserData',fig_ud);
        else
          waitfor(warndlg(sprintf(['Invaild X-marker. Should be a value '...
            'between 0 and %d.'],max(fig_ud.timePts))));
        end
      end
    case 'delete'
      % Open list dialog for deletion of X-marker(s)
      if (length(fig_ud.xMarkers) > 1) | ~isempty(fig_ud.xMarkers(1).value)
        [lststr{1:length(fig_ud.xMarkers)}] = deal(fig_ud.xMarkers.value);
        % Convert to strings
        for i = 1:length(lststr), lststr{i} = num2str(lststr{i}); end
        [result2,result3] = listdlg('PromptString','Current X-markers',...
          'ListSize',[160,180],'Name','Delete X-Marker',...
          'ListString',lststr');
        if result3
          % If the OK button was pressed
          for i = result2, fig_ud.xMarkers(i).value = []; end
          set(handles.fig,'UserData',fig_ud);
        end
      else
        % No X-markers have been set
        waitfor(warndlg('There are currently no X-markers set.'));
      end
    case 'cancel'
      % Do nothing
  end
...
for n=1:length(srs)
  ...
  % If interpolation is delayed, find original point
  if figUD.delayInterp
    x = ceil(origX/imageUD.scale(1));
    y = ceil(origY/imageUD.scale(2));
    z = ceil(origZ/imageUD.scale(3));
  end
...
if isempty(pltLine)
  ...
  % Add the callback for adding X-Markers
  uimenu('Parent',contextMenu_h,'Callback','showsrs2(''AddXMarker_Callback'',get(get(gcbo,''Parent''),''UserData''),gcbo);', ...
    'Label','Add/Delete X-Marker','UserData',0);
  ...
  for n=1:length(srs)
    ...
    if n == 1
      uimenu('Parent',contextMenu_h,'Callback','showsrs2(''ShowTimePlot_Callback'',get(get(gcbo,''Parent''),''UserData''),gcbo);', ...
        'Label',srs_names{n},'Checked',checkedArray{n},'UserData',n,'Separator','On');
    else
      uimenu('Parent',contextMenu_h,'Callback','showsrs2(''ShowTimePlot_Callback'',get(get(gcbo,''Parent''),''UserData''),gcbo);', ...
...
% Update the X-markers
if (length(xMarkers) > 1) | ~isempty(xMarkers(1).value) | ~isempty(xMarkers(1).handle)
  idx = [];
  for n = 1:length(xMarkers)
    % Loop through X-markers and take appropriate action
    if ~isempty(xMarkers(n).value) & isempty(xMarkers(n).handle)
      % Plot a new X-marker
      xMarkers(n).handle = line([xMarkers(n).value,xMarkers(n).value],...
        [1,1],'visible','off');
      idx = [idx,n];
    elseif isempty(xMarkers(n).value) & ~isempty(xMarkers(n).handle)
      % Delete the line
      delete(xMarkers(n).handle);
      xMarkers(n).handle = [];
    else
      % Current line, reset the Y-limits
      set(xMarkers(n).handle,'YData',[1,1],'visible','off');
      idx = [idx,n];
    end
  end
  % Save the handle
  if isempty(idx), figUD.xMarkers = struct('value',[],'handle',[]);
  else, figUD.xMarkers = xMarkers(idx); end
  set(handles.fig,'UserData',figUD);
end
...
% Now update the X-markers Y-limits
if (length(xMarkers) > 1) | ~isempty(xMarkers(1).value)
  for n = 1:length(xMarkers)
    set(xMarkers(n).handle,'YData',get(pltAxes,'YLim'),'visible','on');
  end
end
...
set(get(pltAxes,'title'),'String', ...
  sprintf('Disp (%d,%d,%d)  Orig (%d,%d,%d)',origX,origY,origZ,...
  origCoords(1),origCoords(2),origCoords(3)));
...
if length(pltLine) > 1 & strcmp(get(handles.ShowLegendMenu,'Checked'),'on') & ~isempty(pltLineOn_i)
  ...
  try
    set(legend_h,'Interpreter','none'); % avoid super/sub-script using '_', '^', etc.
  catch
  end
...
handles.UseDelayInterpMenu = uimenu('Parent',handles.ConfigMenu,'Label','Using Delayed Interpolation', ...
  'Checked','Off', 'Tag','UseDelayInterpMenu','Enable','Off');
...
set(handles.fig,'UserData',struct('handles',handles,'resizeInfo',resizeInfo,'otherImgAx',[],'tsAx',[]));
...
%         (0 is translucent, 1 is opaque)
%       overlayInfo.scale is the scale factors for x, y and z directions
%       overlayInfo.clusterSize is the number of adajacent voxels used to
%         define a 2D cluster
%       overlayInfo.maskString is the string which describes which
%         overlay(s) to mask overlay series with
%       overlayInfo.rasflag is the three-letter orientation code for the overlay
...
%       otherInfo.delayInterp is a boolean indicating if delayed
%        interpolation is requested
%       otherInfo.rasflag is a six-letter code indicating the orientation
%                  of the data, or empty string if unknown
%
...
baseInfo = struct('srs',[],'cmap',[],'cmapLim',[],'scale',[1,1,1]);
overlayInfo = struct('srs',[],'cmap',[],'cmapLim',[],'transLim',[],...
  'transLevel',[],'scale',[],'clusterSize',[],'maskString','','rasflag','');
otherInfo = struct('enableReorient',0,'origSlicePlane','','voxelSize',[], ...
  'overlayArgIndex',[],'flipTag',{{}},'imageNames',{{}},...
  'showTimePlot',{{}},'delayInterp',0,'rasflag','');
...
elseif isstruct(varargin{currArg}) % A structure, assign relevant fields to baseCfg
  ...
  currArg = currArg + 1;
elseif ischar(varargin{currArg}) & strcmp(lower(varargin{currArg}),'delayinterp')
  % Second argument is 'DELAYINTERP'
  otherInfo.delayInterp = 1;
  currArg = currArg + 1;
else
...
if nargin < currArg
  ...
  while currArg <= nargin
    ...
    if ischar(varargin{currArg}) & strcmp(lower(varargin{currArg}),'delayinterp')
      % Argument is 'DELAYINTERP'
      otherInfo.delayInterp = 1;
      currArg = currArg + 1;
    else
      ...
      if isstruct(varargin{currArg}) & isfield(varargin{currArg},'data')
        ...
        if isfield(varargin{currArg},'info')
          try
            overlayInfo(nOverlays).rasflag = getorientation(varargin{currArg});
          catch
            overlayInfo(nOverlays).rasflag = '';
          end
      ...
      overlayInfo(nOverlays).clusterSize = [];
      overlayInfo(nOverlays).maskString = '';
      ...
      elseif isstruct(varargin{currArg+1})
        ...
        if isfield(varargin{currArg+1},'clusterSize'),
          overlayInfo(nOverlays).clusterSize = varargin{currArg+1}.clusterSize;
        end
        if isfield(varargin{currArg+1},'maskString'),
          overlayInfo(nOverlays).maskString = varargin{currArg+1}.maskString;
        end
        currArg = currArg + 2;  % Move to next argument pair
      elseif ischar(varargin{currArg+1}) & strcmp(lower(varargin{currArg+1}),'delayinterp')
        % Next argument is 'DELAYINTERP'
        otherInfo.delayInterp = 1;
        currArg = currArg + 2;
      else
...
for curr = 1:length(overlayInfo)
  ...
  if ~isempty(overlayInfo(curr).clusterSize) & ~(length(overlayInfo(curr).clusterSize) == 1 & ...
      isnumeric(overlayInfo(curr).clusterSize))
    emsg = 'overlay cluster size must be a scalar number'; error(emsg);
  end
  if ~isempty(overlayInfo(curr).maskString) & ~ischar(overlayInfo(curr).maskString)
    emsg = 'overlay mask string must be a string';
  end
end
...
if isfield(baseInfo,'info') & size(baseInfo.srs,3) > 1
  ...
  if ~isempty(rasflag)
    % make sure overlays aren't labeled with different orientation than base
    overlayorients = {};
    for overlaynum=1:length(overlayInfo)
      if length(overlayInfo(curr).rasflag) > 0 & ...
          ~strcmp(overlayInfo(curr).rasflag, rasflag)
        overlayorients{end+1} = upper(overlayInfo(curr).rasflag);
      end
    end
    if length(overlayorients) > 0
      overlayorientsstr = ['"' overlayorients{1} '"'];
      for overlayorient=overlayorients(2:end)
        overlayorient = overlayorient{1};
        overlayorientsstr = [overlayorientsstr ', "' overlayorient '"'];
      end
      waitfor(warndlg(sprintf('Overlay orientation flags %s are different from base orientation flag "%s".  \nDisplayed orientation labels may be incorrect.  \n', overlayorientsstr, upper(rasflag)), 'Unable to determine orientation','modal'));
    end
    clear overlayorients, overlaynum;
  ...
  otherInfo.rasflag = upper([rasflag rasflag]);
  if ~isempty(otherInfo.rasflag)
    for rasdim=4:6
      raschar = otherInfo.rasflag(rasdim);
      if raschar == 'R', raschar = 'L';
      elseif raschar == 'L', raschar = 'R';
      elseif raschar == 'A', raschar = 'P';
      elseif raschar == 'P', raschar = 'A';
      ...
      otherInfo.rasflag(rasdim) = raschar;
    end
    clear rasdim;
  end
  clear rasflag;
...
elseif ~isempty(origVoxelSize) & isempty(otherInfo.origSlicePlane)
  ...
  % Go ahead and pass voxel size, if valid, so user doesn't have to specify
  if all(sort(size(origVoxelSize)) == [1 3]) & isnumeric(origVoxelSize) & ...
      isreal(origVoxelSize) | all(origVoxelSize > 0)
    otherInfo.voxelSize = origVoxelSize;
  end
...
elseif ~isempty(origVoxelSize) & ~isempty(otherInfo.origSlicePlane)
  ...
  elseif ~any(strcmp(otherInfo.origSlicePlane,{'Axial','Sagittal','Coronal'}))
    ...
    % Fix the rasflag to account for the reoriented image
    if ~isempty(otherInfo.rasflag)
      otherInfo.rasflag(1:3) = otherInfo.rasflag(otherInfo.orient3DOrig2Native);
      otherInfo.rasflag(4:6) = otherInfo.rasflag(otherInfo.orient3DOrig2Native + 3);
    end
  end
end
...
for curr = 1:length(overlayInfo)
  ...
  % Interpolate before display if necessary and delayInterp not specified
  if any(scale~=1) & ~otherInfo.delayInterp
...
% Grab fast layer {layerNum, cmapLim, transLim, clusterSize, maskString}
% stored in the W+L UserData (see local_initializegui)
...
if strcmp(get(handles.UseTransMenu,'Checked'),'on')
  ...
  for n = layers2display
    ...
    clSize = [];
    maskString = '';
    % Determine transLim, cmapLim & clSize (image_ud.cmap is always used)
    if isempty(fastLayerInfo{1}) | fastLayerInfo{1} ~= n
      ...
      if n ~= 1
        % Overlays
        transLim = image_ud.transLim;
        clSize = image_ud.clusterSize;
        maskString = image_ud.maskString;
      end
    else
      ...
      if n ~= 1
        % Overlays
        transLim = fastLayerInfo{3};
        clSize = fastLayerInfo{4};
        maskString = fastLayerInfo{5};
      end
    end
    ...
    for allImages_h_n = 1:length(allImages_h)
      ...
      if fig_ud.delayInterp
        % Interpolate data before display
        x = ceil(loc(1)/image_ud.scale(1));
        y = ceil(loc(2)/image_ud.scale(2));
        z = ceil(loc(3)/image_ud.scale(3));
        currImageData = eval(ax_ud.getImageString);  % Get the current raw image data
        if any(image_ud.scale~=1)
          % Interpolate using trilinear on adjacent slices
          tempImData = zeros([size(currImageData),2]);
          tempImData(:,:,1) = currImageData;
          % Increment all variables.  EVAL will know which one to work on
          x = x + 1; y = y + 1; z = z + 1;
          % If not on a boundary, find the next slice
          if ~((ax_ud.toScreenPlane(3) == 1 & (x > size(image_ud.srs,1))) | ...
              (ax_ud.toScreenPlane(3) == 2 & (y > size(image_ud.srs,2))) | ...
              (ax_ud.toScreenPlane(3) == 3 & (z > size(image_ud.srs,3))))
            currImageData = eval(ax_ud.getImageString);  % Get the next raw image data
          end
          tempImData(:,:,2) = currImageData;
          % Trilinear requires a double
          if ~isa(tempImData,'double')
            tempImData = double(tempImData);
          end
          tempImData = scale3(tempImData,...
            image_ud.scale(ax_ud.toScreenPlane(2)),...
            image_ud.scale(ax_ud.toScreenPlane(1)),...
            image_ud.scale(ax_ud.toScreenPlane(3)));

          % Find the correct image for display
          currImageData = tempImData(:,:,mod(loc(ax_ud.toScreenPlane(3))-1,...
            image_ud.scale(ax_ud.toScreenPlane(3)))+1);
          clear('tempImData');
        end
      else
        % No interpolation
      ...
      % Mask the overlays if requested
      if ~isempty(maskString)
        mask = local_calcmask(handles,allImages_h(allImages_h_n),...
          loc,maskString);
        currImageData = currImageData .* mask;
      end

      % Find the activation clusters of overlays
      clMask = [];
      if ~isempty(clSize)
        clMask = zeros(size(currImageData));
        clMask(find(currImageData >= cmapLim(1))) = 1;
        clMask = bwareaopen(clMask,clSize);
        currImageData = currImageData .* clMask;
      end
  ...
  for n = 1:length(fig_ud.image_h)
    if isempty(fastLayerInfo{1}) | fastLayerInfo{1} ~= n
      ...
      if n ~= 1
        % Overlays
        transLim(n) = image_ud.transLim;
        clSize{n} = image_ud.clusterSize;
        maskString{n} = image_ud.maskString;
      else
        clSize{n} = [];
        maskString{n} = '';
      end
    else
      % This is a "fast layer" get the transLim, cmapLim, clusterSize and
      % maskString from the fastLayerInfo array.
      cmapLim(n,:) = reshape(fastLayerInfo{2},[1,2]);
      if n ~= 1
        % Overlays
        transLim(n) = fastLayerInfo{3};
        clSize{n} = fastLayerInfo{4};
        maskString{n} = fastLayerInfo{5};
      else
        clSize{n} = [];
        maskString{n} = '';
      end
  ...
  for allImages_h_n = 1:length(allImages_h)
    ...
    for n = layers2display % Loop through layers to be displayed.
      ...
      if ~isempty(clSize{n})
        clMask = zeros(size(currImageData));
        clMask(find(currImageData >= cmapLim(n,1))) = 1;
        clMask = bwareaopen(clMask,clSize{n});
        currImageData = currImageData .* clMask;
      end
...
%------------------------local_createTextLabels-------------------------------
function labels=local_createTextLabels(strings)
% local_createTextLabels - create a text label for each input string
%
%  local_createTextLabels(strings)
%
%  strings - a string label or cell array of string labels
%  labels will be added to current axis (so remember to call
%  axes(h) before calling this).
%
if ~iscell(strings)
  strings={strings};
end
for ind=1:length(strings)
  labels(ind)=text(0,0,strings{ind});
  set(labels(ind),'FontWeight','bold','Tag','RASLabel');
end

%------------------------local_positionRASLabels-------------------------------
function local_positionRASLabels(handles)
% local_positionRASLabels - Set the RAS label positions
%
%  local_positionRASLabels(handles)
%
%  handles - array of handles for four RAS labels (left, right, top, bottom)
%  they will be placed relative to their parent axis.
%  Note that parent axis is an image axis with YDir == 'reverse',
%  and (0,0) is in the upper left, but the text labels are still
%  positioned from lower left.
%
for sideind=1:length(handles)
  side=[0 0 0 0]; % left, right, top, bottom
  side(sideind) = 1;
  raslabel_h=handles(sideind);
  set(raslabel_h,'Units','Normalized');
  set(raslabel_h,'Position', ...
    [(side(3)+side(4))*0.5 + (side(2)*1.0),
    (side(1)+side(2))*0.5 + (side(3)*1.0) ] );
  set(raslabel_h,'Units','Characters');
  pos=get(raslabel_h,'Position');
  pos = pos + [((side(2)-side(1)-0.5)*1.0), ((side(3)-side(4))*0.5), 0];
  set(raslabel_h,'Position',pos);
end
...
% Get primary figure UserData
fig_ud=get(handles.fig, 'UserData');

% Get showsrs2 primary image axis UserData
ax_ud=get(handles.imgAx,'UserData');

% Get other image axis UserData
allAxes_h=[handles.imgAx, fig_ud.otherImgAx];
for axisind=1:length(allAxes_h)
  ud=get(allAxes_h(axisind),'UserData');
  % delete existing RAS labels
  if isfield(ud, 'raslabels')
    for handleind=1:length(ud.raslabels)
      delete(ud.raslabels(handleind));
    end
    clear handleind;
  end
  ud.raslabels=[];
  set(allAxes_h(axisind),'UserData',ud);
  clear ud;
end
clear axisind;
clear allAxes_h;
...
if any(strcmp(currOrientMenuTag,{'AxialOrientMenu','CoronalOrientMenu','SagittalOrientMenu'}))
  ...
  margin=0;
  if ~isempty(fig_ud.rasflag)
    margin=2;
  end
  local_changeNumberAxes(handles,1,1,margin,margin,margin,margin); % Set to single axes
  currPoint=get(handles.Slider,'UserData'); % Find current point

  % Get showsrs2 figure data
  % NOTE: Figure UserData must be re-loaded after local_changeNumberAxes since
  ...
  if ~isempty(fig_ud.rasflag)
    axes(handles.imgAx);
    ax_ud.raslabels=local_createTextLabels({'', '', '', ''});
  end
  ...
  switch currOrientMenuTag
    case 'AxialOrientMenu'
      ...
      if ~isempty(fig_ud.rasflag)
        set(ax_ud.raslabels(1), 'String', fig_ud.rasflag(4));
        set(ax_ud.raslabels(2), 'String', fig_ud.rasflag(1));
        set(ax_ud.raslabels(3), 'String', fig_ud.rasflag(5));
        set(ax_ud.raslabels(4), 'String', fig_ud.rasflag(2));
      end
    case 'CoronalOrientMenu'
      ...
      if ~isempty(fig_ud.rasflag)
        set(ax_ud.raslabels(1), 'String', fig_ud.rasflag(4));
        set(ax_ud.raslabels(2), 'String', fig_ud.rasflag(1));
        set(ax_ud.raslabels(3), 'String', fig_ud.rasflag(6));
        set(ax_ud.raslabels(4), 'String', fig_ud.rasflag(3));
      end
    case 'SagittalOrientMenu'
      ...
      if ~isempty(fig_ud.rasflag)
        set(ax_ud.raslabels(1), 'String', fig_ud.rasflag(5));
        set(ax_ud.raslabels(2), 'String', fig_ud.rasflag(2));
        set(ax_ud.raslabels(3), 'String', fig_ud.rasflag(6));
        set(ax_ud.raslabels(4), 'String', fig_ud.rasflag(3));
      end
  end
...
elseif strcmp(currOrientMenuTag,'ThreePlaneOrientMenu')
  ...
  ax_h=local_changeNumberAxes(handles,2,2,[margin margin; margin 1.8],[margin margin; margin 1.8],[margin margin; margin 8],[margin margin; margin 0]);
  ...
  % Set the getImageString, voxelDataAspect, axis limits, toScreenPlane, ButtonDownFcn in each axis
  raslabels = [];
  % Axial view (lower left)
  if ~isempty(fig_ud.rasflag)
    axes(ax_h(2));
    raslabels=local_createTextLabels({fig_ud.rasflag(4), fig_ud.rasflag(1), fig_ud.rasflag(5), fig_ud.rasflag(2)});
  end
  ...
  set(ax_h(2),'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    ...
    'toScreenPlane',[1 2 3],'raslabels',raslabels));
  local_positionRASLabels(raslabels);
  % Corononal view (upper left)
  if ~isempty(fig_ud.rasflag)
    axes(handles.imgAx);
    raslabels=local_createTextLabels({fig_ud.rasflag(4), fig_ud.rasflag(1), fig_ud.rasflag(6), fig_ud.rasflag(3)});
  end
  ...
  set(handles.imgAx,'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    ...
    'toScreenPlane',[1 3 2],'raslabels',raslabels));
  local_positionRASLabels(raslabels);
  % Saggittal view (upper right)
  if ~isempty(fig_ud.rasflag)
    axes(ax_h(1));
    raslabels=local_createTextLabels({fig_ud.rasflag(5), fig_ud.rasflag(2), fig_ud.rasflag(6), fig_ud.rasflag(3)});
  end
  ...
  set(ax_h(1),'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    ...
    'toScreenPlane',[2 3 1],'raslabels',raslabels));
  local_positionRASLabels(raslabels);
...
delete([figUD.otherImgAx,figUD.tsAx]);
...
figUD.tsAx=[];
if (columns*rows > 1)
  figUD.otherImgAx=ax_h(1:columns*rows-2);
  figUD.tsAx=ax_h(columns*rows-1);
end
set(handles.fig,'UserData',figUD);
clear figUD
...
%------------------local_calcmask-----------------------
function mask=local_calcmask(handles,ax_h,loc,maskString);
% Calculate the mask based on the mask string
...
% Find the image to mask by
n = strmatch(maskString,fig_ud.image_names);
...
if ~isempty(fastLayerInfo{1}) & fastLayerInfo{1} == n
  cmapLim = fastLayerInfo{2};
  clusterSize = fastLayerInfo{4};
else
  cmapLim = image_ud.cmapLim;
  clusterSize = image_ud.clusterSize;
end
...
%                  three dimensions of image_ud.srs
%     timePts(n) - currently selected time point to be displayed
ax_ud = get(get(ax_h,'Parent'),'UserData');
if fig_ud.delayInterp
  % Interpolate data before display
  x = ceil(loc(1)/image_ud.scale(1));
  y = ceil(loc(2)/image_ud.scale(2));
...
% Calculate the mask & cluster if necessary
mask = zeros(size(currImageData));
mask(find(currImageData >= cmapLim(1))) = 1;
if ~isempty(clusterSize)
  mask = bwareaopen(mask,clusterSize);
end
...
%
% $Log: showsrs2.m,v $
% Revision 1.111  2009/04/23 18:58:02  bizzell
% Added overlay by overlay masking options
%
% Revision 1.110  2007/07/18 17:26:09  gadde
% 'Interpreter' property does not exist in earlier MATLAB versions?
%
% Revision 1.109  2007/06/01 18:21:49  gadde
% Add otherImgAx and tsAx in local_initializegui so that if Resize_Callback
% is called before these are added to figUD (which happens on newer versions
% of MATLAB), it won't crash.
%
% Revision 1.108  2007/03/23 18:14:43  gadde
% Turn of TeX interpreter for legend text.
%
% Revision 1.107  2006/11/17 20:39:37  gadde
% Fix rasflag if showsrs2 reorients coronal or sagittal data.
%
% Revision 1.106  2006/10/31 17:06:14  gadde
% Don't get upset if overlays don't have orientation information.
%
% Revision 1.105  2006/10/04 14:05:20  gadde
% Check that base and overlay orientations (if specified) are the same.
%
% Revision 1.104  2006/06/23 20:54:27  gadde
% Fix some RAS label bugs.
%
% Revision 1.103  2006/06/21 19:17:02  gadde
% Put in some missing updates from Josh.
%
% Revision 1.102  2006/06/21 18:05:59  gadde
% From Josh Bizzell:
...
%
% Revision 1.101  2006/06/13 20:17:17  gadde
% Add RAS labels to image display if orientation info is provided.
%
% Revision 1.100  2006/06/02 21:44:51  gadde
