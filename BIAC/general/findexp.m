...
expDB={'\\fatt\data\inactive.txt','\\fatt\data\experiments.txt'};
...
try
  ...
  if ~isempty(cachedExpInfo)
    ...
    while (filename <= length(expDB)) & (validExpInfo == 1)
...
try
  ...
  while 1
    ...
    if line(1) ~= '#'  % Skip lines starting with #
      ...
      [expbasepath,expName]=strtok(line,char(9));
      
      % Store the experiment name and path
      expNames=cat(1,expNames,cellstr(expName(2:end)));
      expPaths=cat(1,expPaths,cellstr([expbasepath,expName(2:end)]));
    end
  end
  fclose(fid);
