...
switch data.current_conversion
    case 'dicom'
        writefile_dir = fullfile(writefile_dir_selected, anon_subdir, 'dicom') ;
    case 'convert'
        switch data.convert_format
            case {'analyze','nifti'}
                writefile_dir = fullfile(writefile_dir_selected, anon_subdir, 'nifti') ;
            otherwise
                error('Convert format could not be determined');
        end
    case 'list'
        writefile_dir = fullfile(writefile_dir_selected, anon_subdir, 'list') ;
    otherwise
...
for n = 1:nfiles
    scan_names(n) = cellstr(get_dicom_fieldname_func(flist(n).name, nfullstopsoffset+2));
end
...
for n = 1:nfiles
    ...
    if strcmp(data.first_scan,'yes') == 1
        scanlistfile = fullfile(data.writefile_dir_selected, anon_subdir,'scan_list.txt');
    ...
    if strcmp(data.current_conversion, 'dicom')
        ...
        switch data.anonymise
            case 'yes'
                ...
                if ((size(findstr(version_number, '(R2006b)'),1) ~= 0) || size(findstr(version_number, '(R2007a)'),1) ~= 0 || (size(findstr(version_number, '(R2007b)'),1) ~= 0))
