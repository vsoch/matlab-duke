...
if length(strfind(search_all_header_func(writefile_th, 'ImageType'),'mosaic')) > 0
...
%   voxel size
dcminfo.PixelSpacing = search_all_header_func(writefile_th, 'PixelSpacing');
        ...
        case '' %   bodges - some MPRAGEs don't have the dimensions set in dcminfo.MRAcquisitionType
            ...
            switch data.current_mosaic_flag
                case 'mosaic'   % behave as 2D
                    dim_nslices = str2num(search_text_header_func(writefile_th, 'sGroupArray.asGroup[0].nSize'));
                    if isvar('dcminfo.SpacingBetweenSlices') == 1
                        dim_slice_thick = dcminfo.SpacingBetweenSlices;
                    else
                        dim_slice_thick = str2num(search_text_header_func(writefile_th, 'sSliceArray.asSlice[0].dThickness'));
                    end
...
if isvar('dcminfo.RepetitionTime')
...
if isvar('dcminfo.InversionTime')
...
%   the DICOM
if isvar('dcminfo.protocol_name') == 1
    protocol_name = dcminfo.ProtocolName;
else
    protocol_name = search_text_header_func(writefile_th, 'tProtocolName');
    protocol_name = strrep(protocol_name, '+','');
    protocol_name = strrep(protocol_name, '-','_');
    protocol_name = strrep(protocol_name, 'AF8','');
    protocol_name = strrep(protocol_name, '"','');
end
...
if isvar('dcminfo.ImageOrientationPatient')
...
%   PE direction
if isvar('dcminfo.InPlanePhaseEncodingDirection') == 1
...
if data.current_dcminfo.BitsAllocated ~= -1
...
data.current_header_pars.protocol_name = protocol_name;
