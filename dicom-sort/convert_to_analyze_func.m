...
vol = process_one_slice_func(data, example_image);
...
for n = 1:nfiles
    ...
    %   convert (mosaic to 3D if necc., swap dimensions, rotate) one slice
    vol = process_one_slice_func(data, single_image);
    if strcmp(data.convert_format, 'analyze')
        fwrite(fp_writefile_4d_img, vol, precision);
    end
    if strcmp(data.convert_format, 'nifti')
        fwrite(fp_writefile_4d_nii, vol, precision);
    end
