...
%   these choices are overwritten by data.* parameters in function calls to dicom_sort_convert_main
    ...
    function data = set_data_defaults
        data.anonymise = 'yes';
        data.test = 'no'; % no = move data to ima_to_delete directory after sort/convert
        data.version_number = '1.15';
...
if (nargin == 0)
    ...
    figure('MenuBar','none',...
        'Name', ['Sort and Convert Siemens DICOM. v' data.version_number ' S.R.'],...
        'NumberTitle','off',...
    ...
    function figure_CreateFcn(obj, eventdata, handles)
        ...
        butt_select_us_dicom = uicontrol(obj,...
            'Style','pushbutton',...
            'String','Select Unsorted DICOM directory',...
        ...
        checkbox_retain_dicom = uicontrol(obj,...
            'Style','CheckBox',...
            'BackgroundColor', [grey_level grey_level grey_level],...
            'String','Keep as DICOM/Retain DICOM copy',...
            'Position',[10,600-115,250,25],...
        ...
        function radiobuttongroup_action(obj,eventdata)
            ...
            if findstr(format_select_string, 'Analyze')
                data.convert_format = 'analyze';
            elseif findstr(format_select_string, 'NIfTI')
                data.convert_format = 'nifti';
            else
