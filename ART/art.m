...
rotat_thresh = .035;%outliers have angular movement > rad_thresh in at least one orientation - change this???
mvmt_diff_thresh = 2.0;%1.0;
rotat_diff_thresh = .035;
...
%                 P{i} = spm_select(Inf,'.*\.nii|.*\.img',['Select functional volumes for session'  num2str(i) ':']);
                P{i} = spm_select(Inf,'.*\.nii|swu.*\.img',['Select functional volumes for session'  num2str(i) ':']);
        end
        if realignfile == 1
