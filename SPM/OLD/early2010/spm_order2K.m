...
if strcmp('SUB_RUNCARDS_SUB','yes')
    ...
    chreg_cards=char(12,105);
    
    f = ceil(i.*rand(12,1));
    for j = 1:12
        if f(j) < 10
            chreg_cards(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV000',num2str(f(j)),'.img,1');
        end
        if f(j) >=10
            if f(j) < 100
                chreg_cards(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV00',num2str(f(j)),'.img,1');
            end
        end
        if f(j) >=100
            chreg_cards(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV0',num2str(f(j)),'.img,1');
        end
    end
...
if strcmp('SUB_RUNFACES_SUB','yes')
    ...
    %This allocates a spot in memory for the array so that it doesn't have
    %to find a new spot for every iteration of the loop. 
    chreg_faces=char(12,105);
    
    i = 195;
    f = ceil(i.*rand(12,1));
    for j = 1:12
        if f(j) < 10
            chreg_faces(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV000',num2str(f(j)),'.img,1');
        end
        if f(j) >=10
            if f(j) < 100
                chreg_faces(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV00',num2str(f(j)),'.img,1');
            end
        end
        if f(j) >=100
            chreg_faces(j,1:105) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV0',num2str(f(j)),'.img,1');
        end
    end
...
if strcmp('SUB_RUNREST_SUB','yes')
    ...
    %This allocates a spot in memory for the array so that it doesn't have
    %to find a new spot for every iteration of the loop.
    chreg_rest=char(12,104);
    
    f = ceil(i.*rand(12,1));
    for j = 1:12
        if f(j) < 10
            chreg_rest(j,1:104) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/rest/swuV000',num2str(f(j)),'.img,1');
        end
        if f(j) >=10
            if f(j) < 100
                chreg_rest(j,1:104) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/rest/swuV00',num2str(f(j)),'.img,1');
            end
        end
        if f(j) >=100
            chreg_rest(j,1:104) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/rest/swuV0',num2str(f(j)),'.img,1');
        end
    end
