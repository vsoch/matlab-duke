%-----------------------------------------------------------------------
% SPM BATCH FIGS - includes spm_batch1.m & spm_batch2.m
%
% These template scripts are filled in and run by a bash script,
% spm_batch.py and spm_batch_TEMPLATE.sh from the head node of BIAC
%
...
%-----------------------------------------------------------------------
% Segment the newly named anatomical
%-----------------------------------------------------------------------

spm('defaults','fmri')
spm_jobman('initcfg');

matlabbatch{1}.spm.spatial.preproc.data = {horzcat('SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/anat/anat.img,1')};
...
if strcmp('SUB_RUNFACES_SUB', 'yes')
    copyfile('SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/anat/c1anat.img','SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces');
    copyfile('SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/anat/c1anat.hdr','SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces');
end
    
if strcmp('SUB_RUNCARDS_SUB', 'yes')
   copyfile('SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/anat/c1anat.img','SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/cards');        
   copyfile('SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/anat/c1anat.hdr','SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/cards');
end
...
matlabbatch{2}.spm.spatial.coreg.estimate.source = {'SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/c1anat.img,1'};
...
%-----------------------------------------------------------------------
matlabbatch{3}.spm.spatial.normalise.estwrite.subj.source = {'SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/c1anat.img,1'};
...
matlabbatch{2}.spm.spatial.coreg.estimate.source = {'SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/cards/c1anat.img,1'};
...
%-----------------------------------------------------------------------
matlabbatch{3}.spm.spatial.normalise.estwrite.subj.source = {'SUB_MOUNT_SUB/Analysis/SPM/Processed/SUB_SUBJECT_SUB/cards/c1anat.img,1'};
matlabbatch{3}.spm.spatial.normalise.estwrite.subj.wtsrc = '';
matlabbatch{3}.spm.spatial.normalise.estwrite.subj.resample = {
...
if strcmp('SUB_RUNCARDS_SUB','yes')
    ...
    chreg_cards=char(12,102);
    
    f = ceil(i.*rand(12,1));
    for j = 1:12
        if f(j) < 10
            chreg_cards(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV000',num2str(f(j)),'.img,1');
        end
        if f(j) >=10
            if f(j) < 100
                chreg_cards(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV00',num2str(f(j)),'.img,1');
            end
        end
        if f(j) >=100
            chreg_cards(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/cards/swuV0',num2str(f(j)),'.img,1');
        end
    end
...
if strcmp('SUB_RUNFACES_SUB','yes')
    ...
    %This allocates a spot in memory for the array so that it doesn't have
    %to find a new spot for every iteration of the loop. 
    chreg_faces=char(12,102);
    
    i = 195;
    f = ceil(i.*rand(12,1));
    for j = 1:12
        horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV000',num2str(f(j)),'.img,1')
        if f(j) < 10
            chreg_faces(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV000',num2str(f(j)),'.img,1');
        end
        if f(j) >=10
            if f(j) < 100
                chreg_faces(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV00',num2str(f(j)),'.img,1');
            end
        end
        if f(j) >=100
            chreg_faces(j,1:102) = horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/swuV0',num2str(f(j)),'.img,1');
        end
    end
...
if strcmp('SUB_RUNFACES_SUB','yes')
    ...
    if isdir('affect')==0
        sprintf('%s','Currently no faces a folder exists.  Creating faces block folder within single subject faces folder.')
        mkdir affect
    end
end
...
% Save the paths to the swu images to be used for the affect design
scans = matlabbatch{1}.spm.stats.fmri_spec.sess.scans;
clear matlabbatch

%--------------------------------------------------------------------------
% SPM AFFECT
%
% Now we will set up an SPM.mat that will run the affect contrasts (since there are
% subject specific designs we can't have the script do it.
%
% Contrasts should be as follows: 
% Faces > Shapes [-2 1 1];
% Fear Faces > Shapes [-1 1];
% Angry Faces > Shapes [-1 0 1];
%--------------------------------------------------------------------------
...
% Copy the design matrices from the manaul runs folder into affect Analyzed
% folder
  
spm('defaults','fmri')
spm_jobman('initcfg');

switch 'SUB_DESIGN_SUB'
    case '4block'
        copyfile('SUB_MOUNT_SUB/Scripts/SPM/Manual_Runs/FIGS_4block.mat','SUB_MOUNT_SUB/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SUB_SUBID_SUB_4block.mat');
        matrix = 'SUB_SUBID_SUB_4block.mat';
    case '4blockmixed'
        copyfile('SUB_MOUNT_SUB/Scripts/SPM/Manual_Runs/FIGS_4blockmixed.mat','SUB_MOUNT_SUB/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SUB_SUBID_SUB_4blockmixed.mat');
        matrix = 'SUB_SUBID_SUB_4blockmixed.mat';
    case 'Individual'
        copyfile('SUB_MOUNT_SUB/Scripts/SPM/Manual_Runs/SUB_SUBID_SUB.mat','SUB_MOUNT_SUB/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SUB_SUBID_SUB_Individual.mat');
        matrix = 'SUB_SUBID_SUB_Individual.mat';
    otherwise
        error('Design name not valid.  Exiting')
end

% Edit the .mat files to add the correct paths to the images and output
% directory
%--------------------------------------------------------------------------
% Faces Affect Run
%--------------------------------------------------------------------------
...
matlabbatch{1}.spm.stats.fmri_spec.sess.scans={};
matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {horzcat(homedir,'Processed/SUB_SUBJECT_SUB/faces/art_regression_outliers_swuV0001.mat')};

for i = 1:195
    matlabbatch{1}.spm.stats.fmri_spec.sess.scans{i} = scans{i};
end
matlabbatch{2}.spm.stats.fmri_est.spmmat = {'SUB_MOUNT_SUB/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SPM.mat'};
matlabbatch{3}.spm.stats.con.spmmat = {'SUB_MOUNT_SUB/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SPM.mat'};

%Runs the single subject pre-processing batch file.
spm_jobman('run_nogui',matlabbatch)

% Update the .mat to have local paths and save a copy of the single subject 
% processing batch within single subject folder under Analyzed
matlabbatch{1}.spm.stats.fmri_spec.dir = {'N:/FIGS.01/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect'};
matlabbatch{1}.spm.stats.fmri_spec.sess.scans={};
matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/art_regression_outliers_swuV0001.mat'};

% Prepare the raw swu images with local paths:
clear scans;
scans = {
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0001.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0002.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0003.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0004.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0005.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0006.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0007.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0008.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0009.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0010.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0011.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0012.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0013.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0014.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0015.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0016.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0017.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0018.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0019.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0020.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0021.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0022.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0023.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0024.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0025.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0026.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0027.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0028.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0029.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0030.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0031.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0032.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0033.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0034.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0035.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0036.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0037.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0038.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0039.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0040.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0041.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0042.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0043.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0044.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0045.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0046.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0047.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0048.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0049.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0050.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0051.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0052.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0053.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0054.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0055.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0056.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0057.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0058.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0059.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0060.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0061.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0062.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0063.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0064.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0065.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0066.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0067.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0068.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0069.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0070.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0071.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0072.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0073.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0074.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0075.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0076.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0077.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0078.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0079.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0080.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0081.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0082.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0083.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0084.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0085.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0086.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0087.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0088.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0089.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0090.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0091.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0092.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0093.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0094.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0095.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0096.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0097.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0098.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0099.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0100.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0101.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0102.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0103.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0104.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0105.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0106.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0107.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0108.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0109.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0110.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0111.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0112.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0113.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0114.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0115.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0116.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0117.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0118.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0119.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0120.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0121.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0122.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0123.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0124.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0125.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0126.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0127.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0128.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0129.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0130.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0131.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0132.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0133.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0134.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0135.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0136.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0137.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0138.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0139.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0140.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0141.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0142.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0143.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0144.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0145.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0146.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0147.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0148.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0149.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0150.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0151.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0152.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0153.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0154.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0155.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0156.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0157.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0158.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0159.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0160.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0161.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0162.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0163.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0164.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0165.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0166.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0167.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0168.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0169.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0170.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0171.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0172.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0173.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0174.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0175.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0176.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0177.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0178.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0179.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0180.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0181.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0182.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0183.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0184.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0185.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0186.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0187.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0188.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0189.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0190.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0191.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0192.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0193.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0194.img,1'
                                                 'N:/FIGS.01/Analysis/SPM/Processed/SUB_SUBJECT_SUB/faces/swuV0195.img,1'
                                                 };

for i = 1:195
    matlabbatch{1}.spm.stats.fmri_spec.sess.scans{i} = scans{i};
end
matlabbatch{2}.spm.stats.fmri_est.spmmat = {'N:/FIGS.01/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SPM.mat'};
matlabbatch{3}.spm.stats.con.spmmat = {'N:/FIGS.01/Analysis/SPM/Analyzed/SUB_SUBJECT_SUB/Faces/affect/SPM.mat'};

save(matrix,'matlabbatch')

clear matlabbatch scans;
...
% also go back and change the paths to the images so that we can
% run analyis on a local machine.
%-----------------------------------------------------------------------

%-------------------------------------------------------------------------
% Cleanup ANATOMICAL RAW IMAGES
%-------------------------------------------------------------------------
...
cd(horzcat(homedir,'Processed/SUB_SUBJECT_SUB'))
if exist('highres','dir')
    cd(horzcat(homedir,'Processed/SUB_SUBJECT_SUB/highres'))
    if exist('raw','dir')
        cd raw
        delete V0*
        cd(horzcat(homedir,'Processed/SUB_SUBJECT_SUB/highres'))
...
if strcmp('SUB_RUNCARDS_SUB', 'yes')
    ...
    % Fixing paths for cards_pfl
    cd(horzcat(homedir,'Analyzed/SUB_SUBJECT_SUB/cards_pfl'))
    spm_change_paths_swd('/ramSUB_MOUNT_SUB/','SUB_MOUNT_SUB/','N:/FIGS.01/','/');

    % Fixing paths for Cards
    cd(horzcat(homedir,'Analyzed/SUB_SUBJECT_SUB/Cards'))
    spm_change_paths('SUB_MOUNT_SUB/','N:/FIGS.01/','/');
