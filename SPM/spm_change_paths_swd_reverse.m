function spm_change_paths_swd_reverse(newswdpath,newpath,oldpath,slash)
%
...
elseif nargin == 4
   ...
   change_swd(newswdpath,newpath,oldpath,slash,old_slash);
   change_P(newpath,oldpath,slash,old_slash);
   change_VY(newpath,oldpath,slash,old_slash);
...
function change_swd(newswdpath,newpath,oldpath,slash,old_slash)
 load SPM.mat SPM;
 size_swd = size(SPM.swd);
 size_oldpath = length(oldpath);
 size_newswdpath = length(newswdpath);
 new_size_swd = size_swd(1,2) - size_oldpath + size_newswdpath;
   for i = 1:new_size_swd
      if i <= size_newswdpath
          temp(i) = newswdpath(i);
          if temp(i) == old_slash
              temp(i) = slash;
          end;
      else
          temp(i) = SPM.swd(size_oldpath-size_newswdpath+i);
          if temp(i) == old_slash
              temp(i) = slash;
          end;
      end
...
function change_P(newpath,oldpath,slash,old_slash)
 load SPM.mat SPM;
...
function change_VY(newpath,oldpath,slash,old_slash)
  load SPM.mat SPM;
